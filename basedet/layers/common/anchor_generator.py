...
from basedet.utils import cached_property

from .function import meshgrid
...
class DefaultAnchorGenerator(BaseAnchorGenerator):
    """
    Default anchor generator for RetinaNet and RPN.
    ...
    def base_anchors(self):
        """Base anchor of one pixel of feature map."""
        return self._different_level_anchors(
            self.anchor_scales.tolist(), self.anchor_ratios.tolist()
        )
...
class AnchorPointGenerator(BaseAnchorGenerator):
    ...
    def generate_anchors_by_features(self, sizes, device):
        ...
        for size, stride in zip(sizes, self.strides):
            grid_x, grid_y = create_anchor_grid(size, self.offset, stride, device)
            grids = F.stack([grid_x, grid_y], axis=1)  # grid shape: (N, 2)
            all_anchors.append(
                F.repeat(
                    F.expand_dims(grids, axis=1), repeats=self.num_anchors, axis=1
                ).reshape(-1, 2)
            )
        return all_anchors


# FIXME: Remove after AnchorPointGenerator becomes faster
class FastPointGenerator:
    """Used to accelarate YOLOX"""

    def __init__(self, strides: tuple = (8, 16, 32)):
        self.strides = strides

    def __call__(self, featmaps):
        all_anchors = []
        for feat, stride in zip(featmaps, self.strides):
            h, w = feat.shape[-2:]
            grid_x, grid_y = meshgrid(F.arange(h), F.arange(w))
            grids = F.stack((grid_x, grid_y), axis=-1).reshape(-1, 2)
            all_anchors.append(grids * stride)
        return all_anchors
